function createWorld(limenis) {
  ...
  for (let i = 0; i < limenis.length; i++) {
    ...
    jaunaSiena.style.transform = `translate3d(${
      600 + limenis[i][0] - limenis[i][6] / 2
    }px, ${400 + limenis[i][1] - limenis[i][7] / 2}px, ${
      limenis[i][2]
    }px) rotateX(${limenis[i][3]}deg) rotateY(${limenis[i][4]}deg) rotateZ(${
      limenis[i][5]
    }deg)`;
