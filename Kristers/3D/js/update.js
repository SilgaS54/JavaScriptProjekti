...
function update() {
  ...
  lode_dx =
    -pirmaLode.vx * Math.cos(pirmaLode.ry * deg) +
    pirmaLode.vy * Math.sin(pirmaLode.ry * deg);
  // lode_dz = -(lode_atrums_x) * Math.sin(lode_ry * deg) - (lode_atrums_y) * Math.cos(lode_ry * deg);
  lode_dz =
    -pirmaLode.vx * Math.sin(pirmaLode.ry * deg) -
    pirmaLode.vy * Math.cos(pirmaLode.ry * deg);
  ...
  lode.style.transform = `translate3d(${600 + pirmaLode.x - 20 / 2}px, ${
    400 + pirmaLode.y - 20 / 2
  }px, ${pirmaLode.z}px) rotateX(${pirmaLode.rx}deg) rotateY(${
    pirmaLode.ry
  }deg) rotateZ(${0}deg)`;
  lode_move -= 1;
  if (lode_move < 0) {
  ...
  for (let sk = 0; sk < manaLode.length; sk++) {
    ...
    manaLode[sk].style.transform = `translate3d(${
      600 + manaLodeData[sk].x - 20 / 2
    }px, ${400 + manaLodeData[sk].y - 20 / 2}px, ${
      manaLodeData[sk].z
    }px) rotateX(${manaLodeData[sk].rx}deg) rotateY(${
      manaLodeData[sk].ry
    }deg) rotateZ(${0}deg)`;
